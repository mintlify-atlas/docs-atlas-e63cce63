...
Ollama does **not** have a Docker Compose file in this repository and must be installed and run separately. The risk-filter pipeline in `packages/core/src/logic/outline/risk.outline.ts` calls the `gpt-oss:120b` model for every parsed signal.

<CardGroup cols={2}>
  <Card title="Install Ollama" icon="download" href="https://ollama.com">
    Follow the official Ollama installation guide for your operating system at
    **ollama.com**.
  </Card>
  <Card title="Pull the required model" icon="robot">
    After installing Ollama, pull the model used by the risk outline:
...
Once Ollama is running, the `getOllama` singleton in `packages/core/src/config/ollama.ts` will connect to it automatically. If you are using a hosted Ollama endpoint instead of a local one, set `CC_OLLAMA_TOKEN` to your Bearer token in `packages/core/.env`.
...
<Tip>
  ...
  CC_MONGO_CONNECTION_STRING=mongodb://host.docker.internal:27017/backtest-pro?wtimeoutMS=15000
  ```
