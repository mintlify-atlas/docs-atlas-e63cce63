---
title: "January 2026 Backtest: LLM Filter Adds +16.68pp PnL"
sidebarTitle: "Backtest Report"
description: "Full trade-by-trade backtest results for January 2026, comparing the baseline signal strategy against the Ollama-gated version."
---
...
The headline result is a **+16.68 percentage-point improvement in total PnL** by skipping just five signals out of twenty-two.
...
The LLM response is validated against a Zod schema that requires:

```typescript
{
  riskAction: z.enum(["skip", "follow"]),
  riskSureLevel: z.number().min(0).max(1),
  riskConfidence: z.number().min(0).max(1),
  riskDescription: z.string(),
  riskReasoning: z.string(),
}
```

Only when `riskAction === "follow"` does the strategy open a position.
