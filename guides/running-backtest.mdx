...
description: "Replay Telegram signals through the local Ollama LLM risk filter over a historical date range and view per-trade results in the backtest-kit UI."
---
...
<Steps>
  ...
  <Step title="signalJobSubject triggers LLM screening">
    ...
    `avgRangePct` and `momentum24hPct`, sends them to `gpt-oss` via Ollama,
